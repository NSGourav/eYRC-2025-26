...
from tf_transformations import quaternion_from_euler   #added
...
def detect_aruco(image):
    ...
    for i in range(len(ids)):
        ...
        # Calculate full orientation from rotation vector
...
class aruco_tf(Node):
    ...
    def process_image(self):
        ...
        for i in range(len(ids)):
            ...
            distance_from_rgb = distance_from_rgb_list[i]  # distance in meters
            ...
            # pitch = 0.0
            # yaw = angle_aruco
            # q_yaw = quaternion_from_euler(roll, pitch, yaw)
            # q_rot = [0.0, -0.7068252, 0.0, 0.7068252]

            # r_yaw = R.from_quat(q_yaw)
            # r_rot = R.from_quat(q_rot)
            # r_combined = r_rot * r_yaw
            ...
            t_obj.child_frame_id = f'{self.team_id}__fertiliser_can_{marker_id}'
