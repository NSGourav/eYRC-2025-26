...
class ShapeDetector3B(Node):
    def __init__(self):
        super().__init__('shape_det_3b')
        self.get_logger().info('Shape Detector 3B Node has been started.')
    ...
    def odom_callback(self,msg:Odometry):
        ...
        _,_,self.position_yaw=euler_from_quaternion([q.x,q.y,q.z,q.w])

    def scan_callback(self, msg:LaserScan):
        # Placeholder for scan data processing logic
        all_pts=self.scan_to_points(msg)
        if len(all_pts)<25:     return
        filtered_pts=self.filter_by_dist(all_pts,2.0)

        binary_img=self.points_to_image(filtered_pts)
        raw_lines=self.detect_lines(binary_img)
        if len(raw_lines)==0: return

        line_clusters=self.group_by_proximity(raw_lines)
        
        best_group = None
        best_score = -1

        for group in line_clusters:
        ...
        if self.flag_print==True:
                ...
                # if  sqrt((self.world_pos[0]-self.position_x)**2 +(self.world_pos[1]-self.position_y)**2) < 10.6:
                if abs(self.world_pos[1]-self.position_y)<0.02:
                    status_msg = String()
                    status_msg.data = f"{self.shape_print},{self.position_x:.2f},{self.position_y:.2f}"
    ...
    #comment below line to avoid visualization window
        self.visualize(filtered_pts, binary_img, best_group, shape_type)
    ...
    def detect_shape(self, lines):
        ...
        if len(lines) == 2 and lines[1]['length']<0.25:
            ...
            angle = 180 - angle
            dist = sqrt((lines[0]['end'][0]-lines[1]['start'][0])**2 +
        ...
        if len(lines) == 4:
            ...
            for i in range(len(lines)-1):
                dist= sqrt((lines[i]['end'][0]-lines[i+1]['start'][0])**2 +
...
## Abstracted processes
    ...
    def median_filter(self, data):
        ...
        return filtered
    def filter_by_dist(self, points, max_dist=1.5):
    ...
    # Lines
    def group_by_proximity(self, lines, threshold=None):
        if threshold is None: threshold=self.merge_distance_tolerance
        if not lines:
            return []
        groups = []
        used = set()
    ...
    def local_to_world(self, local_pos):
        ...
        # Rotation matrix
        cos_yaw = cos(self.position_yaw)
        sin_yaw = sin(self.position_yaw)
