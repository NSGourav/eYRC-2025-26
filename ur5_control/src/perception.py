...
def detect_aruco(image):
    ...
    for i in range(len(detected_ids)):
        ...
        rotation_matrix, _ = cv2.Rodrigues(np.array([rvecs[0][0][1], rvecs[0][0][0], rvecs[0][0][2]]))
        ...
        cv2.drawFrameAxes(image, cam_mat, dist_mat, rvecs[0], tvecs[0], 0.05)

    return center_aruco_list, angle_aruco_list, valid_ids
